<?php
    ...
    class PedidosPorDespachador{
        ...
        public function extraer_pedidos( $fechaI="", $fechaF="",$empleado = ""){
            $d1 = $this->model->extraer_pedidos2($fechaI,$fechaF,$empleado);
            $d2 = $this->model->extraer_info_pedidos($fechaI,$fechaF,$empleado);


            if(count($d1) > 0){
                $f = $this->ordenar_data_mayor_a_menor($this->contar_pedidos($this->ordenar_info($d1)));
                return $this->jsonResponse([$f,$d2],[]);
            }else{

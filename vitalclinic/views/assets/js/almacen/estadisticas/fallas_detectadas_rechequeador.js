...
const mostrar_datos_tabla = (datos) => {
    ...
    if(data.length > 0){
      ...
      data.forEach((element,i) => {
        ...
        $template_body_table_pedidos.querySelector('.empleado').textContent = `${element.nombre_rechequeador}`;
        ...
        tf = tf + Number(element.fallas_detectadas);
        $template_body_table_pedidos.querySelector('.t_fallas').textContent = element.fallas_detectadas;
...
const extraer_estadisticas = async (form) => {
   ...
   if(is_loader) $loader.classList.remove("hidden");
    try {
        const data = await app('http://192.168.0.164/vitalclinic/controllers/almacen/estadisticas/fallas_detectadas_rechequeador.php?extraer_pedidos=1','POST',form);
        is_loader= false;

...
const mostrar_datos_tabla = (datos) => {
    ...
    if(data.length > 0){
      ...
      data.forEach((element,i) => {
        ...
        tp = tp + Number(element.pedidos);
        $template_body_table_pedidos.querySelector('.t_p_rechequeados').textContent = element.pedidos;
        $template_body_table_pedidos.querySelector('.porc_p_r').textContent = `${element.porc_pedidos}%`;
      ...
      d.querySelector('#total_pedidos').innerText = parseInt(tp);
...
const extraer_estadisticas = async (form) => {
   ...
   if(is_loader) $loader.classList.remove("hidden");
    try {
        const data = await app('http://192.168.0.164/vitalclinic/controllers/almacen/estadisticas/mejor_rechequeador.php?extraer_pedidos=1','POST',form);
        is_loader= false;
